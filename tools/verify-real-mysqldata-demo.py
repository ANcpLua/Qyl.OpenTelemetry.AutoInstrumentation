...
def runtime_identifier() -> str:
    ...
    fail(f"unsupported NativeAOT MySql.Data gate platform: {platform.system()} {platform.machine()}")
...
def verify_report(name: str, completed: subprocess.CompletedProcess[str], expected_runtime_mode: str) -> None:
    ...
    if completed.stdout.count("expected-mysqldata-error=InvalidOperationException") != 2:
        fail(f"{name} expected exactly 2 MySql.Data error tokens\nstdout={completed.stdout}")
    ...
    if not isinstance(activities, list) or len(activities) != 2:
        fail(f"{name} expected exactly 2 MySql.Data activities, got {activities!r}")
...
def main() -> None:
    ...
    verify_report("managed MySql.Data demo", managed, "dynamic-code-supported")
    verify_report("NativeAOT MySql.Data demo", nativeaot, "nativeaot")
