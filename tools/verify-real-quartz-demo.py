...
PROJECT = ROOT / "demos" / "Qyl.RealQuartzDemo" / "Qyl.RealQuartzDemo.csproj"
...
def runtime_identifier() -> str:
    ...
    fail(f"unsupported NativeAOT Quartz gate platform: {platform.system()} {platform.machine()}")


def parse_report(stdout: str) -> dict[str, Any]:
    start = stdout.find("{\n")
    if start < 0:
        fail(f"Quartz demo did not emit JSON report\nstdout={stdout}")

    try:
        report = json.loads(stdout[start:])
    except json.JSONDecodeError as exc:
        fail(f"Quartz demo emitted invalid JSON report: {exc}\nstdout={stdout}")

    if not isinstance(report, dict):
        fail(f"Quartz demo report must be a JSON object: {report!r}")
    return report
...
def run_nativeaot(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    executable = output / ("Qyl.RealQuartzDemo.exe" if platform.system().lower() == "windows" else "Qyl.RealQuartzDemo")
    if not executable.exists():
        fail(f"NativeAOT Quartz executable missing: {executable}")
...
def main() -> None:
    ...
    verify_report("managed Quartz demo", managed, "dynamic-code-supported")
    verify_report("NativeAOT Quartz demo", nativeaot, "nativeaot")
    print("real-quartz-demo-ok")
