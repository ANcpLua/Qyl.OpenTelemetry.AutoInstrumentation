...
PROJECT = ROOT / "demos" / "Qyl.RealLog4NetDemo" / "Qyl.RealLog4NetDemo.csproj"
...
def runtime_identifier() -> str:
    ...
    fail(f"unsupported NativeAOT log4net gate platform: {platform.system()} {platform.machine()}")


def parse_report(stdout: str) -> dict[str, Any]:
    start = stdout.find("{\n")
    if start < 0:
        fail(f"log4net demo did not emit JSON report\nstdout={stdout}")

    try:
        report = json.loads(stdout[start:])
    except json.JSONDecodeError as exc:
        fail(f"log4net demo emitted invalid JSON report: {exc}\nstdout={stdout}")

    if not isinstance(report, dict):
        fail(f"log4net demo report must be a JSON object: {report!r}")
    return report
...
def verify_report(name: str, completed: subprocess.CompletedProcess[str], expected_runtime_mode: str) -> None:
    ...
    if "log4net-memory-count=2" not in completed.stdout:
    ...
    if not isinstance(activities, list) or len(activities) != 2:
        fail(f"{name} expected exactly 2 log4net activities, got {activities!r}")
...
def run_nativeaot(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    executable = output / ("Qyl.RealLog4NetDemo.exe" if platform.system().lower() == "windows" else "Qyl.RealLog4NetDemo")
    if not executable.exists():
        fail(f"NativeAOT log4net executable missing: {executable}")
...
def main() -> None:
    ...
    verify_report("managed log4net demo", managed, "dynamic-code-supported")
    verify_report("NativeAOT log4net demo", nativeaot, "nativeaot")
    print("real-log4net-demo-ok")
