...
INTERCEPTOR_CATALOG_CS_PATH = ROOT / "src" / "Qyl.AutoInstrumentation.SourceGenerators" / "QylGeneratedSourceInterceptorCatalog.g.cs"
...
def expected_outputs(contract: dict[str, Any]) -> dict[Path, str]:
    return {
        ...
        INTERCEPTOR_CATALOG_CS_PATH: render_interceptor_catalog_cs(contract),
...
def render_interceptor_catalog_cs(contract: dict[str, Any]) -> str:
    source_interceptor_keys = {
        str(item["key"])
        for item in source_interceptor_signal_items(contract)
        if item.get("qyl_status") == "implemented"
    }
    missing = GENERATED_INTERCEPTOR_CATALOG_REQUIRED_SIGNAL_KEYS - source_interceptor_keys
    if missing:
        fail(f"generated interceptor catalog requires implemented source-interceptor signal keys: {sorted(missing)}")
    ...
    lines = [
        ...
        "            new InterceptorEmissionDescriptor(InterceptorKind.DbCommand, InterceptorEmitterFamily.Database, InterceptorMethodShape.AsyncOrSyncValue, InterceptorSignalOwnership.TraceAndMetric, InterceptorErrorPolicy.Exception, InterceptorDurationPolicy.RuntimeMetric, DbCommandBody: new DbCommandBodyDescriptor(\"DbCommand\", \"command\", \"global::Qyl.AutoInstrumentation.QylInterceptedDbCommand\", \"global::Qyl.AutoInstrumentation.QylInterceptedDbCommand\", \"GetTimestamp\", \"StartActivity\", \"ObserveAsync\", \"RecordSuccess\", \"RecordException\", \"RecordDuration\")),",
