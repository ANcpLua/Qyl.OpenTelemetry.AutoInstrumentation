...
UPSTREAM_CONTRACT_PATH = ROOT / "contracts" / "otel-dotnet-auto.upstream.yaml"
QYL_NATIVE_CONTRACT_PATH = ROOT / "contracts" / "qyl-native-instrumentations.yaml"
OWNERSHIP_PATH = ROOT / "contracts" / "qyl-aot-ownership.yaml"
RESOLVED_CONTRACT_PATH = ROOT / "contracts" / "generated" / "qyl-aot-contract.resolved.yaml"
SCHEMA_PATH = ROOT / "contracts" / "generated" / "qyl-aot-contract.schema.json"
COVERAGE_MATRIX_PATH = ROOT / "contracts" / "coverage-matrix.md"
CONFORMANCE_PLAN_PATH = ROOT / "contracts" / "qyl-aot-autoinstrumentation.conformance-plan.json"
...
def verify_contract_item(item: dict[str, Any]) -> None:
    ...
    if status == "implemented":
        ...
        if lane == "source_interceptor":
            required_tokens = [
                "src/Qyl.Telemetry.AutoInstrumentation.SourceGenerators/QylAutoInstrumentationGenerator.cs",
        ...
        if lane == "runtime_public_telemetry":
            proof_tokens = [
                "src/Qyl.Telemetry.AutoInstrumentation.DiagnosticListeners",
                "DiagnosticListener",
                "src/Qyl.Telemetry.AutoInstrumentation/QylMetricMeters.cs",
                "ILogger",
                "ActivitySource",
                "Meter",
            ]
    ...
    schema = {
        ...
        "properties": {
            ...
            "generated_from": {
                ...
                "properties": {
                    "upstream_contract": {"const": "contracts/otel-dotnet-auto.upstream.yaml"},
                    "qyl_native_contract": {"const": "contracts/qyl-native-instrumentations.yaml"},
                    "qyl_ownership": {"const": "contracts/qyl-aot-ownership.yaml"},
                },
            },
            "contract_items": {
                "type": "array",
...
def render_coverage_matrix(contract: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "This matrix is generated from `contracts/generated/qyl-aot-contract.resolved.yaml`.",
        f"The exact {UPSTREAM_CONTRACT_ITEM_COUNT}-row upstream contract lives in `contracts/otel-dotnet-auto.upstream.yaml`; qyl-native promises live in `contracts/qyl-native-instrumentations.yaml`; qyl ownership and evidence for upstream rows live in `contracts/qyl-aot-ownership.yaml`.",
