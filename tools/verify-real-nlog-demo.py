...
PROJECT = ROOT / "demos" / "Qyl.RealNLogDemo" / "Qyl.RealNLogDemo.csproj"
...
def runtime_identifier() -> str:
    ...
    fail(f"unsupported NativeAOT NLog gate platform: {platform.system()} {platform.machine()}")


def parse_report(stdout: str) -> dict[str, Any]:
    start = stdout.find("{\n")
    if start < 0:
        fail(f"NLog demo did not emit JSON report\nstdout={stdout}")

    try:
        report = json.loads(stdout[start:])
    except json.JSONDecodeError as exc:
        fail(f"NLog demo emitted invalid JSON report: {exc}\nstdout={stdout}")

    if not isinstance(report, dict):
        fail(f"NLog demo report must be a JSON object: {report!r}")
    return report
...
def verify_report(name: str, completed: subprocess.CompletedProcess[str], expected_runtime_mode: str) -> None:
    ...
    if "nlog-memory-count=2" not in completed.stdout:
        fail(f"{name} did not prove disabled Trace filtering and enabled Info/Error logging\nstdout={completed.stdout}")
    ...
    if not isinstance(activities, list) or len(activities) != 2:
        fail(f"{name} expected exactly 2 NLog activities, got {activities!r}")
...
def run_nativeaot(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    executable = output / ("Qyl.RealNLogDemo.exe" if platform.system().lower() == "windows" else "Qyl.RealNLogDemo")
    if not executable.exists():
        fail(f"NativeAOT NLog executable missing: {executable}")
...
def main() -> None:
    ...
    verify_report("managed NLog demo", managed, "dynamic-code-supported")
    verify_report("NativeAOT NLog demo", nativeaot, "nativeaot")
    print("real-nlog-demo-ok")
