...
PROJECT = ROOT / "demos" / "Qyl.RealSqliteDemo" / "Qyl.RealSqliteDemo.csproj"
...
def runtime_identifier() -> str:
    ...
    fail(f"unsupported NativeAOT SQLite gate platform: {platform.system()} {platform.machine()}")


def parse_report(stdout: str) -> dict[str, Any]:
    start = stdout.find("{\n")
    if start < 0:
        fail(f"SQLite demo did not emit JSON report\nstdout={stdout}")

    try:
        report = json.loads(stdout[start:])
    except json.JSONDecodeError as exc:
        fail(f"SQLite demo emitted invalid JSON report: {exc}\nstdout={stdout}")

    if not isinstance(report, dict):
        fail(f"SQLite demo report must be a JSON object: {report!r}")
    return report
...
def verify_report(name: str, completed: subprocess.CompletedProcess[str], expected_runtime_mode: str) -> None:
    ...
    if not isinstance(activities, list) or len(activities) != 4:
        fail(f"{name} expected exactly 4 SQLite activities, got {activities!r}")
...
def run_nativeaot(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    executable = output / ("Qyl.RealSqliteDemo.exe" if platform.system().lower() == "windows" else "Qyl.RealSqliteDemo")
    if not executable.exists():
        fail(f"NativeAOT SQLite executable missing: {executable}")
...
def main() -> None:
    ...
    verify_report("managed SQLite demo", managed, "dynamic-code-supported")
    verify_report("NativeAOT SQLite demo", nativeaot, "nativeaot")
    print("real-sqlite-demo-ok")
