...
REQUIRED_PACKAGE_ENTRIES = {
    "analyzers/dotnet/cs/Qyl.Telemetry.AutoInstrumentation.SourceGenerators.dll",
    "build/Qyl.Telemetry.AutoInstrumentation.targets",
    "buildTransitive/Qyl.Telemetry.AutoInstrumentation.targets",
}
...
def pack_runtime(feed: Path, env: dict[str, str]) -> Path:
    ...
    package = feed / f"Qyl.Telemetry.AutoInstrumentation.{version}.nupkg"
...
def verify_targets(name: str, text: str) -> None:
    for token in [
        ...
        "<InterceptorsNamespaces>$(InterceptorsNamespaces);Qyl.Telemetry.AutoInstrumentation.Generated</InterceptorsNamespaces>",
    ]:
    ...
    for token in [
        ...
        "Qyl.Telemetry.AutoInstrumentation.InterceptsLocationAttribute.g.cs",
...
def verify_package(package: Path) -> None:
    with zipfile.ZipFile(package) as archive:
        ...
        build_targets = archive.read("build/Qyl.Telemetry.AutoInstrumentation.targets").decode("utf-8")
        transitive_targets = archive.read("buildTransitive/Qyl.Telemetry.AutoInstrumentation.targets").decode("utf-8")
