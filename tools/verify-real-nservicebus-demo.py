...
PROJECT = ROOT / "demos" / "Qyl.RealNServiceBusDemo" / "Qyl.RealNServiceBusDemo.csproj"
...
def parse_report(stdout: str) -> dict[str, Any]:
    start = stdout.find("{\n")
    if start < 0:
        fail(f"NServiceBus demo did not emit JSON report\nstdout={stdout}")

    try:
        report = json.loads(stdout[start:])
    except json.JSONDecodeError as exc:
        fail(f"NServiceBus demo emitted invalid JSON report: {exc}\nstdout={stdout}")

    if not isinstance(report, dict):
        fail(f"NServiceBus demo report must be a JSON object: {report!r}")
    return report
...
def verify_report(name: str, completed: subprocess.CompletedProcess[str], expected_runtime_mode: str) -> None:
    ...
    for token in [
        "published=alpha",
        "sent-and-handled=beta",
        "expected-nservicebus-error=no-route",
    ]:
...
def run_managed(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    assembly = PROJECT.parent / "bin" / "Release" / TARGET_FRAMEWORK / "Qyl.RealNServiceBusDemo.dll"
...
def main() -> None:
    ...
    verify_report("managed NServiceBus demo", managed, "dynamic-code-supported")
    print("real-nservicebus-demo-ok")
