...
def runtime_identifier() -> str:
    ...
    fail(f"unsupported NativeAOT Elastic.Transport gate platform: {platform.system()} {platform.machine()}")


def parse_report(stdout: str) -> dict[str, Any]:
    start = stdout.find("{\n")
    if start < 0:
        fail(f"Elastic.Transport demo did not emit JSON report\nstdout={stdout}")

    try:
        report = json.loads(stdout[start:])
    except json.JSONDecodeError as exc:
        fail(f"Elastic.Transport demo emitted invalid JSON report: {exc}\nstdout={stdout}")

    if not isinstance(report, dict):
        fail(f"Elastic.Transport demo report must be a JSON object: {report!r}")
    return report
...
def verify_report(name: str, completed: subprocess.CompletedProcess[str], expected_runtime_mode: str) -> None:
    ...
    if not isinstance(activities, list) or len(activities) != 2:
        fail(f"{name} expected exactly 2 Elastic.Transport activities, got {activities!r}")
...
def run_nativeaot(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    executable = output / ("Qyl.RealElasticTransportDemo.exe" if platform.system().lower() == "windows" else "Qyl.RealElasticTransportDemo")
    if not executable.exists():
        fail(f"NativeAOT Elastic.Transport executable missing: {executable}")
...
def main() -> None:
    ...
    verify_report("managed Elastic.Transport demo", managed, "dynamic-code-supported")
    nativeaot = run_nativeaot(env)
    verify_report("NativeAOT Elastic.Transport demo", nativeaot, "nativeaot")
    print("real-elastictransport-demo-ok")
