...
PROJECT = ROOT / "demos" / "Qyl.RealElasticsearchDemo" / "Qyl.RealElasticsearchDemo.csproj"
...
def runtime_identifier() -> str:
    ...
    fail(f"unsupported NativeAOT Elasticsearch gate platform: {platform.system()} {platform.machine()}")


def parse_report(stdout: str) -> dict[str, Any]:
    start = stdout.find("{\n")
    if start < 0:
        fail(f"Elasticsearch demo did not emit JSON report\nstdout={stdout}")

    try:
        report = json.loads(stdout[start:])
    except json.JSONDecodeError as exc:
        fail(f"Elasticsearch demo emitted invalid JSON report: {exc}\nstdout={stdout}")

    if not isinstance(report, dict):
        fail(f"Elasticsearch demo report must be a JSON object: {report!r}")
    return report
...
def verify_report(name: str, completed: subprocess.CompletedProcess[str], expected_runtime_mode: str) -> None:
    ...
    if completed.stdout.count("expected-elasticsearch-error=TransportException") != 2:
        fail(f"{name} expected exactly 2 Elasticsearch error tokens\nstdout={completed.stdout}")
    ...
    if not isinstance(activities, list) or len(activities) != 2:
        fail(f"{name} expected exactly 2 Elasticsearch activities, got {activities!r}")
...
def run_nativeaot(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    executable = output / ("Qyl.RealElasticsearchDemo.exe" if platform.system().lower() == "windows" else "Qyl.RealElasticsearchDemo")
    if not executable.exists():
        fail(f"NativeAOT Elasticsearch executable missing: {executable}")
...
def main() -> None:
    ...
    verify_report("managed Elasticsearch demo", managed, "dynamic-code-supported")
    nativeaot = run_nativeaot(env)
    verify_report("NativeAOT Elasticsearch demo", nativeaot, "nativeaot")
    print("real-elasticsearch-demo-ok")
