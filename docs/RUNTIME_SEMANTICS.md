...
| ASP.NET Core | `Microsoft.AspNetCore` listener; `HttpContext` payload on `Microsoft.AspNetCore.Hosting.HttpRequestIn.Stop`; .NET 10 built-in metrics are collected from the framework meters. | Real local .NET 10 Kestrel proof covers 204 route response and unhandled-exception 500 response. Route source is `RouteEndpoint.RoutePattern.RawText`; `Request.Path` remains privacy-gated. For .NET 10 ASP.NET Core metrics, the contract source is the built-in `Microsoft.AspNetCore.Components` meter and `aspnetcore.components.navigation`; qyl must not re-emit that metric from `NavigationManager.NavigateTo` because the required component type and route attributes are not available from the source-visible call-site. | Traces: `http.request.method`, `http.route`, `http.response.status_code`, `error.type` for 5xx. Built-in metrics: `aspnetcore.components.navigation` with framework-owned `aspnetcore.components.type`, `aspnetcore.components.route`, and optional `error.type`. | Real managed + NativeAOT trace proof; .NET 10 metrics are framework-owned built-in meter collection, not qyl re-emission. |
