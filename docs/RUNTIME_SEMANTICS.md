...
| WCF client | Source-generated interceptors on source-visible `ClientBase<T>` operation methods; no DiagnosticListener involved. | Real managed proof covers sync and async client calls that fail with `CommunicationException` against an unavailable endpoint. NativeAOT is blocked by `System.ServiceModel` client internals: after contract metadata is rooted, channel creation still uses `DispatchProxy` dynamic code generation and throws `PlatformNotSupportedException`. | `rpc.system=dotnet_wcf`, `rpc.method`, `error.type` on failure; raw endpoint URL is not emitted. | Real managed proof; NativeAOT blocked by WCF client `DispatchProxy` dynamic-code boundary. |
...
| log4net | Source-generated interceptors on source-visible `ILog` logging calls; no DiagnosticListener involved. | Real log4net 3.3 proof covers disabled `Debug` filtering plus enabled `Info`/`Error` logging through a `MemoryAppender`. NativeAOT requires app-side trimmer roots for `log4net` and `System.Configuration.ConfigurationManager`; without them log4net fails in configuration/repository initialization due trimmed parameterless constructors. | `log.severity`; raw log body remains gated and is not emitted by default. | Real managed + NativeAOT proof, with explicit log4net/System.Configuration app-side trimmer-root boundary. |
...
| NServiceBus | Source-generated interceptors on `IMessageSession.Publish`/`Send`; no DiagnosticListener involved. Metrics registration includes qyl's `NServiceBus.Core` meter and NServiceBus' incoming-pipeline meter `NServiceBus.Core.Pipeline.Incoming`. | Real NServiceBus 9.2.11 endpoint proof on the LearningTransport with assembly scanning disabled and explicit handler registration: publish, routed send with a real handler round-trip, and an unrouted send failure (`error.type=Exception`; NServiceBus throws the base exception type for missing routes). NativeAOT is structurally blocked by NServiceBus itself: endpoint creation unconditionally constructs a Reflection.Emit proxy creator (`MessageMapper` → `ConcreteProxyCreator`), which cannot run under NativeAOT. Vendor-confirmed: the NServiceBus 10.2.0 announcement (discuss.particular.net topic 4626) calls scanning-free registration "a foundation for aligning with ahead-of-time compilation and trimming strategies in the future, even though full AOT and trimming compliance is not yet available." | `messaging.system=nservicebus`, `messaging.operation.type=send`, `messaging.operation.name=publish`/`send`, `error.type` on failure; `nservicebus.messaging.operation.duration` from qyl source-visible sends, plus vendor incoming-pipeline metrics when the app emits them. | Real managed proof; NativeAOT is an explicit NServiceBus library boundary (Reflection.Emit at endpoint creation); tracked upstream: [Particular/NServiceBus#7817](https://github.com/Particular/NServiceBus/issues/7817). |
...
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation/Qyl.OpenTelemetry.AutoInstrumentation.csproj -c Release -o /tmp/qyl-pack
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation.DiagnosticListeners/Qyl.OpenTelemetry.AutoInstrumentation.DiagnosticListeners.csproj -c Release -o /tmp/qyl-pack
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation.EntityFrameworkCore/Qyl.OpenTelemetry.AutoInstrumentation.EntityFrameworkCore.csproj -c Release -o /tmp/qyl-pack
# A temp consumer with PackageReference=Qyl.OpenTelemetry.AutoInstrumentation.EntityFrameworkCore and no qyl
...
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation/Qyl.OpenTelemetry.AutoInstrumentation.csproj -c Release -o /tmp/qyl-pack
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation.DiagnosticListeners/Qyl.OpenTelemetry.AutoInstrumentation.DiagnosticListeners.csproj -c Release -o /tmp/qyl-pack
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation.Hosting/Qyl.OpenTelemetry.AutoInstrumentation.Hosting.csproj -c Release -o /tmp/qyl-pack
# A temp consumer with PackageReference=Qyl.OpenTelemetry.AutoInstrumentation.Hosting and no qyl startup call
...
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation/Qyl.OpenTelemetry.AutoInstrumentation.csproj -c Release -o /tmp/qyl-pack
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation.DiagnosticListeners/Qyl.OpenTelemetry.AutoInstrumentation.DiagnosticListeners.csproj -c Release -o /tmp/qyl-pack
dotnet pack src/Qyl.OpenTelemetry.AutoInstrumentation.SqlClient/Qyl.OpenTelemetry.AutoInstrumentation.SqlClient.csproj -c Release -o /tmp/qyl-pack
# A temp consumer with PackageReference=Qyl.OpenTelemetry.AutoInstrumentation.SqlClient and no qyl startup call
...
# 1 error span from an unrouted send. NativeAOT is not run: NServiceBus 9.2.11 endpoint creation
