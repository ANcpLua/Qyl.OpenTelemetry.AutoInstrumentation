...
public sealed partial class QylAutoInstrumentationGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // EXPERIMENTAL (RSEXPERIMENTAL007): emit the implemented-signal contract gate as a
        // compilation-native symbol in the PRE-COMPILATION phase, then bind it back in the standard
        // phase. Replaces the in-process InstrumentationContract.TryGetImplementedSignal lookup.
        context.RegisterPreCompilationSourceOutput(
            context.AdditionalTextsProvider.Collect(),
            static (precompilation, _) => precompilation.AddSource(
                "QylContractRegistry.g.cs",
                SourceText.From(EmitContractRegistrySource(), Encoding.UTF8)));

        // Bind the pre-compilation symbol from the augmented compilation; the compilation is the
        // cache. A newline-joined string keeps the pipeline value equatable for incremental reuse.
        var implementedContractKeys = context.CompilationProvider.Select(static (compilation, _) =>
        {
            var registry = compilation.GetTypeByMetadataName(
                "Qyl.OpenTelemetry.AutoInstrumentation.Generated.QylContractRegistry");
            if (registry is null)
                return string.Empty;
            ...
            return string.Join(
                ...
                registry.GetMembers()
                    .OfType<IFieldSymbol>()
                    .Where(static field => field.HasConstantValue && field.ConstantValue is string)
                    .Select(static field => (string)field.ConstantValue!)
        ...
        context.RegisterSourceOutput(interceptedInvocations.Combine(implementedContractKeys), EmitInterceptors);
    }

    private static string EmitContractRegistrySource()
    {
        var builder = new StringBuilder();
        builder.AppendLine("// <auto-generated/> EXPERIMENTAL pre-compilation phase (RSEXPERIMENTAL007).");
        builder.AppendLine("namespace Qyl.OpenTelemetry.AutoInstrumentation.Generated;");
        builder.AppendLine("internal static class QylContractRegistry");
        builder.AppendLine("{");

        var index = 0;
        foreach (var key in InstrumentationContract.ImplementedSignalKeys)
            builder.AppendLine($"    public const string ImplementedKey{index++} = \"{key}\";");

        builder.AppendLine($"    public const int ImplementedKeyCount = {InstrumentationContract.ImplementedSignalKeys.Length};");
        builder.AppendLine("}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Contract gating moved to emission time (EmitInterceptors), where the pre-compilation
        // QylContractRegistry symbol is bound. Every structurally-matched target now flows through
        // the pipeline; the contract filter is applied once the bound key set is available.
    ...
    private static bool IsSupportedTarget(in InterceptorTarget target, HashSet<string> implementedKeys)
    {
        if (!implementedKeys.Contains(target.ContractKey))
            return false;
        ...
        {
            if (!implementedKeys.Contains(contractKey))
                return false;
        }
    ...
    private static void EmitInterceptors(
        ...
        (ImmutableArray<InterceptedInvocation?> Invocations, string ImplementedKeysJoined) input)
    {
        var nullableInvocations = input.Invocations;
        if (nullableInvocations.IsDefaultOrEmpty)
            return;

        var implementedKeys = input.ImplementedKeysJoined.Length is 0
            ? new HashSet<string>(StringComparer.Ordinal)
            : new HashSet<string>(input.ImplementedKeysJoined.Split('\n'), StringComparer.Ordinal);

        var invocations = nullableInvocations
            .Where(static invocation => invocation is not null)
            .Select(static invocation => invocation!.Value)
            .Where(invocation => IsSupportedTarget(invocation.Target, implementedKeys))
            .Distinct()
