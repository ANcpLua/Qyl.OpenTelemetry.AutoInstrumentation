...
/// <c>Qyl.OpenTelemetry.AutoInstrumentation.Internal.QylSemConvRegistry</c> that bakes every known key into a
...
public sealed class SemConvRegistryGenerator : IIncrementalGenerator
{
    ...
    {
        if (!string.Equals(compilation.AssemblyName, "Qyl.OpenTelemetry.AutoInstrumentation", StringComparison.Ordinal))
