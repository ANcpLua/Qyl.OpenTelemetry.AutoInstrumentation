...
| **M11 CI pipeline** | Make enforcement remote: a GitHub Actions workflow on linux-x64 that installs the substrate, builds the plugin, runs `gate.sh --strict` on every push — also validates a NEW matrix cell (T032). Prereq: vendor the semconv `.nupkg` into the repo (the local feed is an absolute machine path today) | gates pass in CI | n/a | **defined** — alts: NuGet-package the plugin · breadth fan-out (§07) · bytecode path (§T013) |
| M12+ | defined only after M11 proven | — | — | locked |
