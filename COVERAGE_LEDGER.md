...
| **M5 Unknown path (real data)** | Prove the `verdict=unknown` metric slice end-to-end on a REAL non-semconv key (so far only verdict=ok on real data; unknown seen only in the synthetic self-test) — emit a span carrying a key ∉ the 922-key registry, confirm counter records verdict=unknown ≥ 1 | M1 trace golden UNCHANGED; metric shows verdict=unknown ≥ 1 | app-observable identical | **defined** (awaiting go) |
| M6+ | defined only after M5 proven | — | — | locked |
