...
| **M4 Self-telemetry** | Surface conformance as qyl-owned telemetry: emit a metric (`qyl.semconv.unknown_attribute.count`) via a qyl MeterProvider when an emitted key ∉ registry — exercises the METRICS pipeline (M1–M3 are traces-only) + SelfTelemetry | M1 golden span UNCHANGED + new golden for the self-telemetry metric | app-observable identical | **defined** (awaiting go) |
| M5+ | defined only after M4 proven | — | — | locked |
