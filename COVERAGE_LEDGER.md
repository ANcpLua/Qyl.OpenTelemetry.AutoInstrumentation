...
| **M9 MCP span** | Second differentiator: a qyl-authored `mcp.*` client span. MCP semconv is very new — expect some keys ∉ the 1.41-based registry → real verdict=unknown, demonstrating conformance value on a frontier domain | M1 trace golden UNCHANGED + golden | app-observable identical | **defined** |
| M10+ | defined only after M9 proven | — | — | locked |
