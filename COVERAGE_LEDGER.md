...
| **M8 First qyl instrumentation** | Breadth begins: a qyl-authored `ActivitySource` emitting a semconv-shaped span for a real domain — default a **GenAI** span (`gen_ai.*`, a differentiator), conformance-checked + gated via gate.sh | M1 trace golden UNCHANGED + golden for the domain span | app-observable identical | **defined** — alts: MCP span, or a §07 module (HttpClient/SqlClient) |
| M9+ | defined only after M8 proven | — | — | locked |
