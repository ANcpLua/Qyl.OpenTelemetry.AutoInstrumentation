...
| **M6 Logs signal** | Third pillar: prove the LOGS pipeline — an `ILogger` record flows through the substrate with trace correlation (trace_id/span_id injected from the active span) | M1 trace golden UNCHANGED + log-record golden (body + trace_id present) | app-observable identical | **defined** (awaiting go) |
| M7+ | defined only after M6 proven | — | — | locked |
