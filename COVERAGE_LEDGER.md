...
| **M7 Gate runner** | Consolidate the deploy + control/attach + golden-diff + conformance steps into a reusable `spike/gate.sh <fixture>` so future milestones are reproducible, not bespoke bash — hardens the method before §07 breadth scales | M1–M6 all re-pass via the script | n/a (tooling) | **defined** — alt: skip to a differentiator (GenAI/MCP span) or a real §07 module |
| M8+ | defined only after M7 proven | — | — | locked |
